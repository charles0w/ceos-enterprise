...
export async function GET(req: NextRequest) {
  ...
  // Never let a downstream throw turn into an HTML error page — always JSON.
  try {
    ...
    // 2) Search for context snippets (ranked like recall.mjs).
    if (q !== null) {
      ...
      const results = notes.map((n) => ({
        ...
        title: n.title,
        kind: n.kind,
        snippet: n.snippet || (n.body.length > 320 ? `${n.body.slice(0, 320)}…` : n.body),
        score: n.score,
        tags: n.tags,
        updatedAt: n.updatedAt,
      }));
      return NextResponse.json({ query: q, count: results.length, results }, { headers: noStore });
    }

    // 3) List the index (optionally by kind).
    if (list !== null) {
      const notes = await listMemory(list || undefined);
      return NextResponse.json({ count: notes.length, notes }, { headers: noStore });
    }
    ...
    return NextResponse.json({ error: 'context-query-failed', detail: String(e) }, { status: 500 });
  }
}
