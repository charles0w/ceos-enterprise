...
4. **Prompt the editor** ("30s 9:16 cut, hook first, captions, music under it"). It replies + updates the Edit plan panel.
5. **Render** — draft 540p to preview fast, final 1080p to ship. Download lands as `<title>-1080x1920.mp4`, ready for IG/TikTok upload.
6. **Publish to cloud** (optional, needs `CLOUDINARY_API_SECRET`) — uploads the render to Cloudinary (`ceos-social/renders/`) and saves the link on the project. Open `/social` on your phone → grab the file → post. Per-asset **↑ cloud** likewise backs up sources (`ceos-social/library/`) so plans render on any device.
